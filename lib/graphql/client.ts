...
async function fetchProductsFromAPI(limit: number = 6): Promise<SponsoredProduct[]> {
...
async function fetchProductByHandleFromAPI(handle: string): Promise<SponsoredProduct | null> {
  const res = await fetch(MOCK_SHOP_GRAPHQL, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
...
export { fetchProductsFromAPI as getSponsoredProducts };
export { fetchProductByHandleFromAPI as getSponsoredProductByHandle };
